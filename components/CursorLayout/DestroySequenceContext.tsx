...
export function DestroySequenceProvider({ children }: { children: React.ReactNode }) {
  ...
  const startSequence = useCallback(() => {
    ...
    const t2 = setTimeout(() => setPhase('matrix'), HACK_DURATION_MS + COLLAPSE_DURATION_MS);
    const t3 = setTimeout(() => setPhase('loading'), HACK_DURATION_MS + COLLAPSE_DURATION_MS + MATRIX_DURATION_MS);
    const t4 = setTimeout(
      () => setPhase('blackout'),
    ...
    const t5 = setTimeout(
      () => setPhase('recovered'),
      HACK_DURATION_MS + COLLAPSE_DURATION_MS + MATRIX_DURATION_MS + LOADING_DURATION_MS + BLACKOUT_DURATION_MS
    );
